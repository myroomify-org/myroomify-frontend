...
export class GuestProfile implements OnInit {
  ...
  saveProfile() {
    this.profileApi.editProfile$(this.user).subscribe({
      ...
      error: (error:any) => this.failed("Profile update failed")
    })
  }
  ...
  openCalendar(isCheckout: boolean, booking: any) {
    this.isChoosingCheckout = isCheckout
  }
  ...
  editBooking(booking: any) {
    ...
    this.bookingApi.editBooking$(booking.id, updatedData).subscribe({
      next: () => {
        this.success("Booking has been updated")
        this.editingBookingId = null
      },
      error: (err) => {
        this.failed("Booking failed")
      }
    })
  }
  ...
  cancelEdit(booking: any) {
    if (this.originalBookingData) {
      booking.check_in = this.originalBookingData.check_in;
      booking.check_out = this.originalBookingData.check_out;
    }
    this.editingBookingId = null;
